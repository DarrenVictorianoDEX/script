...
def main():
    ...
    if args.debug:
        ...
        kube.update_fluentd_image_for_namespace_list(image=image, NS_List=namespace_list, debug=True, boot=args.boot)
    else:
        print("------- REAL RUN -------")
        print(f"Updating {len(namespace_list)} Job/s listed below:")
        for i in namespace_list:
            print(f'- {i}')
        print(f"Image to update to: {image} \n")
        kube.update_fluentd_image_for_namespace_list(image=image, NS_List=namespace_list, debug=False, boot=args.boot)
